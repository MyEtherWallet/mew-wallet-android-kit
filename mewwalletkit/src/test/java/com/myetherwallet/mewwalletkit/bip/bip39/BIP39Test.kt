...
import com.myetherwallet.mewwalletkit.bip.bip39.wordlist.CustomWordlist
import com.myetherwallet.mewwalletkit.bip.bip39.wordlist.EnglishWordlist
import com.myetherwallet.mewwalletkit.bip.bip39.wordlist.JapaneseWordlist
...
class BIP39Test {
    ...
    private val customLanguage = CustomWordlist(
        arrayOf(
            "aband0n",
            "ab1l1ty",
            "abl3",
    ...
    @Test
    fun `Should pass all test vectors`() {
        for (vector in testVectors) {
            println("Entropy " + vector.entropy)
            val entropyBIP39 = BIP39(vector.entropy.hexToByteArray(), vector.language)
            val mnemonicBIP39 = BIP39(vector.mnemonic, vector.language)

            Assert.assertEquals(entropyBIP39.mnemonic, vector.mnemonic)
            Assert.assertArrayEquals(mnemonicBIP39.entropy as ByteArray, vector.entropy.hexToByteArray())

            if (vector.language is JapaneseWordlist) {
                val password = "㍍ガバヴァぱばぐゞちぢ十人十色"
                val normalizedPassword = "メートルガバヴァぱばぐゞちぢ十人十色"

                val entropySeedPassword = entropyBIP39.seed(password)
                val entropySeedNormalizedPassword = entropyBIP39.seed(normalizedPassword)
                val mnemonicSeedPassword = mnemonicBIP39.seed(password)
                val mnemonicSeedNormalizedPassword = mnemonicBIP39.seed(normalizedPassword)

                Assert.assertArrayEquals(entropySeedPassword as ByteArray, vector.seed.hexToByteArray())
                Assert.assertArrayEquals(entropySeedNormalizedPassword as ByteArray, vector.seed.hexToByteArray())
                Assert.assertArrayEquals(mnemonicSeedPassword as ByteArray, vector.seed.hexToByteArray())
                Assert.assertArrayEquals(mnemonicSeedNormalizedPassword as ByteArray, vector.seed.hexToByteArray())
            } else {
                val entropySeed = entropyBIP39.seed(vector.password ?: "")
                val mnemonicSeed = mnemonicBIP39.seed(vector.password ?: "")
                Assert.assertArrayEquals(entropySeed as ByteArray, vector.seed.hexToByteArray())
                Assert.assertArrayEquals(mnemonicSeed as ByteArray, vector.seed.hexToByteArray())
            }
        }
    }

    private data class TestVector(
        val language: BaseWordlist,
        val entropy: String,
        ...
        val mnemonic: List<String> by lazy { mnemonicString.split(" ", "　") }
    }
}